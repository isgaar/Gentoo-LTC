...
grep -Fqx 'PORTAGE_MAKEOPTS="-j6 -l6"' "$profile"
grep -Fqx 'PORTAGE_EMERGE_DEFAULT_OPTS="--jobs=1 --load-average=6 --with-bdeps=y"' "$profile"
grep -Fq 'create_classic_single_disk_layout swap=20GiB' "$profile"
grep -Fqx 'LIVE_ZRAM_ENABLED=true' "$profile"
grep -Fqx 'LIVE_ZRAM_SIZE="4G"' "$profile"
grep -Fq 'function configure_live_zram_swap()' "$functions"
grep -Fq 'zramctl --find --size' "$functions"
grep -Fq 'configure_live_zram_swap' "$functions"
